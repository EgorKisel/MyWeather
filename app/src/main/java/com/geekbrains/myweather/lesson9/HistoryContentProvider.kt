...
import com.geekbrains.myweather.domain.room.*
import java.util.*
...
class HistoryContentProvider: ContentProvider() {
    ...
    override fun query(
        ...
        val cursor = when(uriMatcher.match(uri)) {
            ...
            else -> { throw IllformedLocaleException ("произошло что-то странное") }
        }
        context?.let {
            cursor.setNotificationUri(it.contentResolver, contentUri)
        }
    ...
    override fun getType(uri: Uri): String? {
        return when (uriMatcher.match(uri)){
            ...
            URI_ID -> {
                ...
                throw IllformedLocaleException ("произошло что-то странное")
            }
        }
    }
